...
if ($_SESSION['is_auth'] == 1) {
    ...
    if ((count($_POST) > 0) && isset($_POST['post-id']) && isset($_POST['comment'] )) {
        ...
        if (empty($form['errors'])) {
            ...
            secure_query($con, $add_comment_query, 'isss', $user['id'], $post_id, $current_time, $_POST['comment'], );
            $URL = '/post.php?id='.$post_id;
            header("Location: $URL");
        }
    }
    $page_content = include_template('post-details.php', ['content_types' => $content_types, 'form_values' => $form['values'], 'form_errors' => $form['errors'], 'field_error_codes' => $field_error_codes, 'form_type' => $form_type, 'user' => $user]);
    print($page_content);
    exit();
}
